...
EXPECTED_VERSION="1.9.27"
EXPECTED_ZIP_SIZE_BYTES="3597158"
EXPECTED_ZIP_SHA256="0af7e69fe75e602feb371c87a13bdde21e75f9e86cce8f4f5675cc3b00f69348"
ZIP_PATH="public/downloads/sycm-keyword-collector-v${EXPECTED_VERSION}.zip"
DMP_VERSION="2.3.47"
DMP_ZIP_SHA256="6b3d32c093ddbfc9a0d35bc4e404299d9f7475349dd27718d854b57de50df664"
...
require_fixed 'sizeLabel: "约 3.6 MB"' src/app/tools/page.tsx "工具页插件包大小标识不一致"
...
[[ "$package_readme" != *"1.9.26"* && "$package_readme" != *"1.9.25"* && "$package_readme" != *"1.9.23"* ]] || fail "包内 README 仍含旧版本状态"
...
require_fixed 'function overviewRoiValue(model, side)' public/tools/dmp-report-engine/report-engine.js "达摩盘报告总览缺少 ROI 专用计算"
require_fixed 'const calculated = completenessEngine.safeIntervalDivide(paidGmv, spend, {' public/tools/dmp-report-engine/report-engine.js "达摩盘报告总览 ROI 未按付费成交额除以推广消耗"
require_fixed 'const metric = (side, key) => modelCell(key === "roi"' public/tools/dmp-report-engine/report-engine.js "达摩盘报告总览未使用 ROI 专用呈现"
...
unzip -p "$DMP_ZIP_PATH" '*report-engine.js' | grep -F 'function overviewRoiValue(model, side)' >/dev/null || fail "达摩盘安装包报告总览缺少 ROI 专用计算"
unzip -p "$DMP_ZIP_PATH" '*report-engine.js' | grep -F 'const calculated = completenessEngine.safeIntervalDivide(paidGmv, spend, {' >/dev/null || fail "达摩盘安装包报告总览 ROI 公式不正确"
unzip -p "$DMP_ZIP_PATH" '*report-engine.js' | grep -F 'const metric = (side, key) => modelCell(key === "roi"' >/dev/null || fail "达摩盘安装包报告总览未使用 ROI 专用呈现"
