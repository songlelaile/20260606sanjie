...
DMP_VERSION="2.3.13"
DMP_ZIP_SHA256="6443ad2df90862dc47557163095d48495f652209ae7ef14536bb7dc9f947415e"
...
require_fixed 'data-report-quality="partial"' src/components/tools/DmpGrowthReportViewer.tsx "达摩盘成长报告缺少部分数据提示"
require_fixed "数据完整性提示" src/lib/dmp-report-export.ts "达摩盘导出缺少部分数据提示"
...
unzip -p "$DMP_ZIP_PATH" 'html-writer.js' | grep -F -- '--table-body:#edf5f3' >/dev/null || fail "达摩盘本机报告缺少定版浅色表格背景"
unzip -p "$DMP_ZIP_PATH" 'html-writer.js' | grep -F '.table-shell tbody td{background-color:var(--table-body)}' >/dev/null || fail "达摩盘本机报告正文不是定版浅绿色"
...
unzip -p "$DMP_ZIP_PATH" '*rolling7-html-writer.js' | grep -F 'name: "细分赛道数据状态"' >/dev/null || fail "达摩盘类目报告缺少赛道数据完整性状态"
